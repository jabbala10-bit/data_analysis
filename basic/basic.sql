...
-- 9. COHORT RETENTION & CHURN ANALYSIS
-- =====================================================
-- Formula: cohort month, retention rate, churn cohorts
-- Use Case: Understand how user retention changes over time and identify at-risk cohorts
--
-- Optimization Strategy:
--   - Build cohorts once, then aggregate for retention curves
--   - Use date keys and precomputed calendar tables where possible
--   - Avoid repeated joins on raw event logs

WITH cohort_activity AS (
    SELECT 
        customer_id,
        FORMAT(first_purchase_date, 'yyyy-MM') AS cohort_month,
        DATEDIFF(MONTH, first_purchase_date, order_date) AS months_since_first_purchase,
        COUNT(*) AS activity_count
    FROM sales s
    JOIN customers c ON s.customer_id = c.customer_id
    WHERE first_purchase_date >= '2023-01-01'
    GROUP BY customer_id, FORMAT(first_purchase_date, 'yyyy-MM'), DATEDIFF(MONTH, first_purchase_date, order_date)
)
SELECT 
    cohort_month,
    ...
    ROUND(
        COUNT(DISTINCT customer_id) * 100.0 / NULLIF(
            (SELECT COUNT(DISTINCT customer_id)
             FROM cohort_activity base
             WHERE base.cohort_month = cohort_activity.cohort_month
               AND base.months_since_first_purchase = 0), 0),
        2) AS retention_pct
FROM cohort_activity
GROUP BY cohort_month, months_since_first_purchase
ORDER BY cohort_month, months_since_first_purchase;

-- 10. FUNNEL CONVERSION & STEP-BY-STEP DROP-OFF
-- =====================================================
-- Formula: funnel counts, conversion ratios
-- Use Case: Measure conversion across website or product flows and identify major drop-off points
--
-- Optimization Strategy:
--   - Aggregate events per user before summarizing funnel steps
--   - Use conditional aggregation to avoid multiple scans
--   - Partition by segment to compare performance across groups
...
SELECT
    ...
    ROUND(COUNT(signup_date) * 100.0 / NULLIF(COUNT(*), 0), 2) AS signup_rate_pct,
    ROUND(COUNT(purchase_date) * 100.0 / NULLIF(COUNT(signup_date), 0), 2) AS purchase_rate_pct
FROM funnel
GROUP BY country
ORDER BY purchase_rate_pct DESC;

-- 11. INVENTORY, SUPPLY, AND DEMAND FORECASTING
-- =====================================================
-- Formula: moving averages, month-over-month momentum
-- Use Case: Evaluate inventory usage trends, demand spikes, and restocking needs
--
-- Optimization Strategy:
--   - Pre-aggregate by SKU and period before applying window functions
--   - Partition by SKU to parallelize forecasts
--   - Use rolling windows on aggregated data, not raw transaction logs
...
WITH sku_monthly AS (
    ...
    GROUP BY sku, FORMAT(order_date, 'yyyy-MM')
)
SELECT
    sku,
    year_month,
    monthly_quantity,
    LAG(monthly_quantity) OVER (PARTITION BY sku ORDER BY year_month) AS prior_month_quantity,
    monthly_quantity - LAG(monthly_quantity) OVER (PARTITION BY sku ORDER BY year_month) AS change_quantity,
    ROUND(
        (monthly_quantity - LAG(monthly_quantity) OVER (PARTITION BY sku ORDER BY year_month)) * 100.0
        / NULLIF(LAG(monthly_quantity) OVER (PARTITION BY sku ORDER BY year_month), 0),
        2) AS momentum_pct
FROM sku_monthly
ORDER BY sku, year_month;

-- 12. DATA QUALITY & ANOMALY DETECTION
-- =====================================================
-- Formula: validation checks, outlier detection
-- Use Case: Identify bad data, duplicates, and extreme values before analysis
--
-- Optimization Strategy:
--   - Run lightweight validation queries first to detect common issues
--   - Use aggregated statistics to flag anomalies instead of row-by-row comparisons
--   - Store quality results in summary tables for repeat execution
...
JOIN monthly_stats ms ON s.year_month = ms.year_month
WHERE s.amount > ms.avg_amount + 3 * ms.stddev_amount
GROUP BY s.year_month
ORDER BY s.year_month;

-- 13. MULTI-SOURCE RECONCILIATION & BALANCE CHECKS
-- =====================================================
-- Formula: UNION ALL + GROUP BY + difference calculation
-- Use Case: Compare records from different systems, detect mismatches, and reconcile totals
--
-- Optimization Strategy:
--   - Summarize each source independently then compare
--   - Use source tags and UNION ALL instead of full cross-source joins
--   - Focus on key metrics and count mismatches, not every field
...
FROM (
    SELECT 'erp' AS source, transaction_id, amount FROM erp_transactions
    UNION ALL
    SELECT 'crm' AS source, transaction_id, amount FROM crm_transactions
    UNION ALL
    SELECT 'billing' AS source, transaction_id, amount FROM billing_transactions
) all_transactions
GROUP BY source
ORDER BY source;
